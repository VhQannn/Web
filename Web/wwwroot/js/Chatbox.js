...
async function notifyTyping(conversationId) {
    await connectionChat.invoke("NotifyTyping", `Conversation-${currentConversationId}`);
}

// Hàm thông báo khi ngưng nhập
async function notifyCancelTyping(conversationId) {
    await connectionChat.invoke("NotifyCancelTyping", `Conversation-${currentConversationId}`);
}
